...
public class IphoneTreeView extends ExpandableListView implements
	...
	public interface IphoneTreeHeaderAdapter {
		...
		/**
		 * Gets the state of the group press
		 * 
		 * @param groupPosition
		 * @return
		 */
	...
	/**
	 * mHeaderViewVisible must be true for views that are displayed in the list header
	 */
	...
	/**
	 * Click on the HeaderView to trigger the event
	 */
	...
	public static final int COLLAPSED = 0;//show status
	public static final int EXPANDED = 1;//A state of
	/**
	 * 
	 * onGroupClick
	 */
	...
	public boolean onGroupClick(ExpandableListView parent, View v,
		...
		if (mAdapter.getHeadViewClickStatus(groupPosition) == COLLAPSED) {
			...
			smoothScrollToPosition(groupPosition);// update position
		...
		// Return true that the click event was handled. Do not pass it forward. Otherwise
		return true;
	...
	@Override
	/**
	 * This method is called when the list interface is updated
	 */
	protected void dispatchDraw(Canvas canvas) {
		super.dispatchDraw(canvas);
		if (mHeaderViewVisible) {
			// The group bar is drawn directly into the interface
